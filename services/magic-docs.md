# Service Prompt — Magic Docs

> Source: `src/services/MagicDocs/prompts.ts`

Auto-documentation service that updates "Magic Doc" files based on conversation learnings. Supports `{{variable}}` template substitution.
...
Your ONLY task is to use the Edit tool to update the documentation file if there is substantial new information to add, then stop. You can make multiple edits - make all Edit tool calls in parallel in a single message. If there's nothing substantial to add, simply respond with a brief explanation and do not call any tools.
